def count_garden_plot(filepath):
    ...
    for block in dependence.values():
        if len(block) == 1 and block[0] not in cannot_be_disintegrated:
            cannot_be_disintegrated.append(block[0])

    return len(table) - len(cannot_be_disintegrated)
